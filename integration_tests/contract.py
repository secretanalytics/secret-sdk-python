...
def main():
    ...
    send_scrt_tx = secret .tx.broadcast(tx)
    print('send scrt:\n', send_scrt_tx)

    # pool query
    pool_query = secret .wasm.contract_query('secret16krcdrqh6y6pazvkj58nrvkerk0q0ttg22kepl', {"pool": {}})
    print('pool query scrt/sscrt:\n', pool_query)

    # convert scrt / sscrt
    addr = test1.key.acc_address
    execute_msg = secret .wasm.contract_execute_msg(addr, 'secret1s7c6xp9wltthk5r6mmavql4xld5me3g37guhsx', {'deposit': {}},
                                              Coins.from_str('1000000uscrt'))

    convert_scrt_sscrt = test1.create_and_sign_tx([execute_msg], fee=fee)
    convert_scrt_sscrt_tx = secret .tx.broadcast(convert_scrt_sscrt)

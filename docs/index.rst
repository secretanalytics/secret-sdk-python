.. Secret SDK documentation master file, created by
   sphinx-quickstart on Sat Feb  6 06:21:18 2021.
   You can adapt this file completely to your liking, but it should at least
   contain the root `toctree` directive.

Introduction
============

.. image:: img/logo.png
    :align: center
    

Welcome to the official documentation for the Python version of the Secret SDK. The Secret SDK
is a simple library toolkit for building software that can interacts with the Secret blockchain.

.. Unfamiliar with Secret? → `Check out Secret Docs`_

.. toctree::
    :maxdepth: 1
    :caption: Get Started

    tutorial

.. toctree::
    :maxdepth: 1
    :caption: Usage Guides

    guides/async
    guides/transactions
    guides/smart_contracts
    guides/custom_key
    guides/troubleshooting

.. .. toctree::
    :maxdepth: 1
    :caption: Examples / Recipes

..    recipes/pseudo_feeder

.. toctree::
    :maxdepth: 1 
    :caption: SDK Reference

    common/strings
    common/numeric
    common/coin_coins
    guides/lcdclient
    common/keys_wallet
    common/exceptions

.. toctree::
    :maxdepth: 1 
    :caption: Core Modules Reference
    
    core_modules/auth
    core_modules/bank
    core_modules/distribution
..    core_modules/gov
    core_modules/market
    core_modules/mint
    core_modules/msgauth
    core_modules/oracle
    core_modules/params
    core_modules/slashing

    core_modules/staking
..    core_modules/supply

    core_modules/tendermint
..    core_modules/treasury

    core_modules/tx
    core_modules/wasm



Indices and tables
==================

* :ref:`genindex`
* :ref:`modindex`
* :ref:`search`
